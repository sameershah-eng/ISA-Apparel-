        ...
        <div className="max-w-7xl mx-auto px-4 md:px-12 flex items-center relative h-10 md:h-12">
          ...
          <div className="flex-1 flex items-center justify-end">
            <nav className="flex items-center space-x-4 md:space-x-12">
              <button 
                onClick={onCartClick} 
                className={`relative group flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.3em] transition-all duration-700 ${cartPulse ? 'text-emerald-600 scale-110' : 'text-[#2C3468]'}`}
              >
                ...
                <div className="md:hidden relative">
                   ...
                   {cartCount > 0 && <span className="absolute -top-1 -right-1 bg-[#2C3468] text-white text-[7px] w-3 h-3 rounded-full flex items-center justify-center font-bold">{cartCount}</span>}
                </div>
              </button>
              <button 
