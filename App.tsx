...
import { Fabrics, Tailoring, Accessories, ReadyToWear, Sale } from './views/StaticPages';
...
const WhatsAppButton: React.FC = () => (
  ...
  >
    ...
    <span className="absolute right-full mr-4 bg-[#2C3468] text-white px-4 py-2 rounded-lg text-[10px] font-bold opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap shadow-xl border border-white/20 uppercase tracking-[0.2em]">
      WhatsApp Concierge
    </span>
  </a>
);
...
const App: React.FC = () => {
    ...
    switch (path) {
      ...
      default:
        if (path.startsWith('#/product/')) {
          ...
          return <ProductDetail slug={slug} onAddToCart={addToCart} key={`product-${slug}`} />;
        }
        return <Home />;
    }
  };
  ...
  return (
    <div className={`min-h-screen flex flex-col ${isCartOpen ? 'cart-drawer-open' : ''}`}>
      ...
      <main className="flex-1 pt-24 md:pt-40 transition-all duration-300">
